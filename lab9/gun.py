import math
import numpy as np
from random import choice, randint

import pygame


FPS = 30

RED = 0xFF0000
BLUE = 0x0000FF
YELLOW = 0xFFC91F
GREEN = 0x00FF00
MAGENTA = 0xFF03B8
CYAN = 0x00FFCC
BLACK = (0, 0, 0)
WHITE = 0xFFFFFF
GREY = 0x7D7D7D
GAME_COLORS = [RED, BLUE, YELLOW, GREEN, MAGENTA, CYAN]
g = np.array([0, 10])

WIDTH = 800
HEIGHT = 600


class Ball:
    def __init__(self, screen: pygame.Surface, x=40., y=450.):
        """ Конструктор класса ball

        Args:
        x - начальное положение мяча по горизонтали
        y - начальное положение мяча по вертикали
        """
        self.screen = screen
        self.r_c = np.array([x, y])
        self.r = 10
        self.v = np.zeros(2)
        self.color = choice(GAME_COLORS)
        self.live = 30

    def move(self, dt):
        """Переместить мяч по прошествии единицы времени.

        Метод описывает перемещение мяча за один кадр перерисовки. То есть,
обновляет значения
        self.x и self.y с учетом скоростей self.vx и self.vy, силы гравитации,
действующей на мяч,
        и стен по краям окна (размер окна 800х600).
        """
        if self.r_c[1] + self.r > HEIGHT and self.v[1] > 0:
            self.v *= np.array([0.7, -0.7])
            self.r_c[1] = HEIGHT - self.r
        if self.r_c[0] + self.r > WIDTH and self.v[0] > 0:
            self.v *= np.array([-0.7, 0.7])
            self.r_c[0] = WIDTH - self.r
        if np.linalg.norm(self.v) < 1:
            self.r_c[1] = HEIGHT - self.r
        else:
            self.r_c += self.v*dt + g*dt**2/2
            self.v += g*dt

    def draw(self):
        """Рисует шарик"""
        pygame.draw.circle(
            self.screen,
            self.color,
            self.r_c,
            self.r
        )

    def hittest(self, obj):
        """Функция проверяет сталкивалкивается ли данный обьект с целью,
описываемой в обьекте obj.

        Args:
            obj: Обьект, с которым проверяется столкновение.
        Returns:
            Возвращает True в случае столкновения мяча и цели. В противном
случае возвращает False.
        """
        rc_t = obj.rc
        r_t = obj.r
        if np.linalg.norm(self.r_c - rc_t) < self.r + r_t:
            return True
        else:
            return False


class Gun:
    def __init__(self, screen):
        self.screen = screen
        self.f2_power = 10
        self.f2_on = 0
        self.an = 1
        self.color = GREY
        self.position = np.array([20, 450])

    def fire2_start(self, event):
        """Вызывается, когда игрок нажал на кнопку мыши.
Соответствует началу стрельбы"""
        self.f2_on = 1

    def fire2_end(self, event):
        """Выстрел мячом.

        Происходит при отпускании кнопки мыши.
        Начальные значения компонент скорости мяча vx и vy зависят от положения
мыши.
        """
        global balls, bullet
        bullet += 1
        new_ball = Ball(self.screen)
        new_ball.r += 5
        position = np.array(event.pos)
        radius_vector = position - new_ball.r_c
        self.an = math.atan2(radius_vector[1], radius_vector[0])
        new_ball.v = self.f2_power * np.array([
            math.cos(self.an),
            math.sin(self.an)
        ])
        balls.append(new_ball)
        self.f2_on = 0
        self.f2_power = 10

    def targetting(self, event):
        """Прицеливание. Зависит от положения мыши."""
        if event:
            delta_y = event.pos[1]-450
            delta_x = event.pos[0]-20
            if delta_x == 0:
                pass
            else:
                self.an = math.atan(delta_y / delta_x)
        if self.f2_on:
            self.color = RED
        else:
            self.color = GREY

    def draw(self):
        """Рисует пистолет"""
        pygame.draw.line(
            self.screen,
            self.color,
            tuple(self.position),
            tuple(self.position + (1 + self.f2_power)*np.array([
                np.cos(self.an),
                np.sin(self.an)])),
            10
        )
        pygame.display.update()

    def power_up(self):
        if self.f2_on:
            if self.f2_power < 100:
                self.f2_power += 1
            self.color = RED
        else:
            self.color = GREY


class Target:
    def __init__(self, screen):
        self.rc = np.array([
            randint(600, 780),
            randint(300, 550)
        ])
        self.r = randint(5, 50)
        self.color = RED

    def draw(self):
        """Отрисовка цели"""
        pygame.draw.circle(
            screen,
            self.color,
            tuple(self.rc),
            self.r
        )


pygame.init()
screen = pygame.display.set_mode((WIDTH, HEIGHT))
bullet = 0
balls = []

clock = pygame.time.Clock()
gun = Gun(screen)
target = Target(screen)
finished = False

while not finished:
    screen.fill(WHITE)

    dt = clock.tick(FPS)

    target.draw()
    for b in balls:
        b.draw()
        if b.hittest(target):
            target = Target(screen)
        b.live -= dt/150
        if b.live <= 0:
            balls.remove(b)
    gun.draw()
    pygame.display.update()

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            finished = True
        elif event.type == pygame.MOUSEBUTTONDOWN:
            gun.fire2_start(event)
        elif event.type == pygame.MOUSEBUTTONUP:
            gun.fire2_end(event)
        elif event.type == pygame.MOUSEMOTION:
            gun.targetting(event)

    for b in balls:
        b.move(dt/100)
    gun.power_up()

pygame.quit()
